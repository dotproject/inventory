<?php /* INVENTORY $Id: addedit.php,v 1.31 2003/11/02 17:29:15 dylan_cuthbert Exp $ */
		...
		<TABLE BORDER="1" CELLPADDING="4" WIDTH="100%">
			<TR>
				<TD>
					...
					<SELECT NAME="inventory_brand" ONCHANGE="EnableDisable( 'brand' );">
						<?php
				...
				<BR /><BR />
					...
					<SELECT NAME="inventory_category" ONCHANGE="EnableDisable( 'category' );">
						<?php
		...
		<TABLE BORDER="1" CELLPADDING="4" WIDTH="100%" >
			<TR>
				<TD>
					...
					<SELECT NAME="inventory_company" CLASS="text">
					<?php
						$compsql = "
...
<!--					<INPUT TYPE="TEXT" ID="invdept_id" NAME="inventory_department" VALUE="<?php echo $obj->inventory_department; ?>" HIDDEN />-->
<!--					<SPAN CLASS="text" ID="invdept_name">--><?php
/*						if ( $obj->inventory_department )
						{
							$deptsql = "
							SELECT dept_id, dept_name
							FROM departments
							WHERE dept_id = {$obj->inventory_department}
							";
							$rows = db_loadList( $
						}
						else echo $AppUI->_( "click to set" );
*/					
					?><!--</SPAN>-->
					<SELECT NAME="inventory_department" CLASS="text">
					<?php
						$deptsql = "
						SELECT dept_id, dept_name
						FROM departments
						...
						if (($rows = db_loadList( $deptsql, NULL )))
						{
							foreach ($rows as $row)
							{
								if ( !getDenyRead( "departments", $row[ 'dept_id' ] ) )
								{
									echo "<OPTION VALUE='".$row["dept_id"].(($row[ "dept_id" ] == $obj->inventory_department ) ? "' SELECTED>":"'>").$row["dept_name"];
								}
							}
						}
					?>
